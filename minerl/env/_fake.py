...
class _FakeEnvMixin(object):
    ...
    def step(
        ...
        for actor_name in self.task.agent_names:
            self._process_action(actor_name, action[actor_name])
        # TODO: Abstract the malmo communication out of the step function.p

        return fobs, reward, terminated, truncated, monitor
...
class _FakeSingleAgentEnv(_FakeEnvMixin, _SingleAgentEnv):
    ...
    def step(self, action):
        ...
        s, reward, terminated, truncated, info = super().step(multi_agent_action)
        return s[aname], reward[aname], terminated, truncated, info[aname]
