...
class _SingleAgentEnv(_MultiAgentEnv):
    ...
    def reset(
        ...
        multi_obs, info = super().reset(seed=seed, options=options)
        return multi_obs[self.task.agent_names[0]], info
    ...
    def step(
        ...
        obs, rew, terminated, truncated, info = super().step(multi_agent_action)

        return obs[aname], rew[aname], terminated, truncated, info[aname]
